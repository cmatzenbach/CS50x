...
// helper functions
// String Lower function - credit to Noir at http://cboard.cprogramming.com/c-programming/88987-strcmp-problem-capital-letters.html
...
char* indexes(const char* path)
{
    // handle HTML index pages
    char* ext_html = "index.html";
    // allocate memory for string to build path in
    char *final_path1 = calloc(1, sizeof(*final_path1) * (strlen(path) + strlen(ext_html) + 1));
    if(final_path1 == NULL) {
        error(500);
        return NULL;
    }
    // add end of string character, then move original path, then html index
    *final_path1 = '\0';
    strcat(strcat(final_path1, path), ext_html);
    
    // handle PHP index pages
    char* ext_php = "index.php";
    // allocate memory for string to build path in
    char *final_path2 = calloc(1, sizeof(*final_path2) * (strlen(path) + strlen(ext_php) + 1));
    if(final_path2 == NULL) {
        error(500);
        return NULL;
    }
    // add end of string character, then move original path, then php index
    *final_path2 = '\0';
    strcat(strcat(final_path2, path), ext_php);
    
    if(access(final_path1, F_OK) == 0) {
        free(final_path2);
        return final_path1;
    }
    else if(access(final_path2, F_OK) == 0) {
        free(final_path1);
        return final_path2;
    }
    else {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *content = malloc(sizeof(char) * (*length));
    if (*content == NULL) {
        error(500); // if you cant allocate more memory, throw internal server error
        return false;
    }
...
const char* lookup(const char* path)
{
    ...
    char* mimetyp_list[] = {".css","text/css",".html","text/html",".gif","image/gif",".ico","image/x-icon",".jpg","image/jpeg",
    ".js","text/javascript",".php","text/x-php",".png","image/png",".html","text/html"};
    
    for (int i = 0; i < 18; i += 2) {
        if (strcasecmp(ext,mimetyp_list[i]) == 0) {
            mimetyp = mimetyp_list[i+1];
            return mimetyp;
        }
        else if (i == 16) {
            error(501);
...
{
    // check method and return 405 error if != GET
    if(strncmp(line,"GET ",4) != 0) {
        error(405);
        return false;
    }
    
    // make sure request-target starts with / - if not throw 501 error
    if(line[4] != '/') {
        error(501);
        return false;
    }
    
    // get the request target and version in a string together, then get just version
    char* req_ver = strchr(line, '/');
    char* version = strchr(req_ver, ' ');
    
    // check if invalid version format or incorrect version 
    if(strchr(version + 1, ' ') != NULL || strstr(version, "HTTP/1.1") == NULL) {
        error(505);
        return false;
    }
    
    // declare space to hold request - will need as much space as from beginning of request to space after
    char request[version - req_ver];
    // move from req+ver string into request, number of spaces from / to space following
    strncpy(request, req_ver, version - req_ver);
    // turn last char (space) into end of string
    request[version - req_ver] = '\0';
    
    // check that there isn't a " in request
    if(strchr(request, '\"') != NULL) {
        error(400);
        return false;
    }
    
    // get query if present
    char* query_find = strchr(request, '?');
    
    // see if query exists, and if so, handle it
    if(query_find != NULL) {
        // declare request target path without query
        char path[query_find - request + 1];
        strncpy(path, request, query_find - request);
        path[query_find - request + 1] = '\0';
        
        // now we have our path and query and can pass to pointer args given
        strcpy(abs_path, path);
        strcpy(query, query_find + 1);
    }
    // if no query
    else {
        // pass path and query (pass query as empty string)
